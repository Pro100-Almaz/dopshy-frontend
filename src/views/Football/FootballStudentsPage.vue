...
import {
  ...
  setFootballStudentSubscribed,
} from '@/services/football'
import type { FootballStudent } from '@/services/football'

const currentPageTitle = 'Футбол: ученики'
const query = ref('')
const studentMode = ref<'unsubscribed' | 'subscribed'>('unsubscribed')
const students = ref<FootballStudent[]>([])
...
function groupName(student: FootballStudent): string {
...
async function load() {
  loading.value = true
  try {
    students.value = await listFootballStudents(studentMode.value === 'subscribed')
    error.value = ''
  } catch (e) {
    error.value = e instanceof Error ? e.message : 'Failed to load football students'
  } finally {
    loading.value = false
  }
}

async function subscribeStudent(student: FootballStudent) {
  savingStudentIds.value.add(student.id)
  try {
    await setFootballStudentSubscribed(student.id, true)
